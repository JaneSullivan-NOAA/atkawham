...
input <- prepare_wham_input(model_name = 'Initial',
                            ...
                            # FLAG! Jim, what are the M assumptions in amak? arep$Mest what are these values?
                                ...
                                selectivity = list(model = c('age-specific', 'age-specific'),
                                                   initial_pars =
                                              list(scales::rescale(colMeans(arep$sel_fsh_1[,-c(1,2)])),#23:46
                                                   scales::rescale(colMeans(arep$sel_ind_1[,-c(1,2)]))),
                                                   # arep$sel_ind_1[1,-c(1,2)]),
                                                   fix_pars = list(1:11,1:11),
                                                   # fix_pars = list(6:7, 8),
                                                   n_selblocks = 2),
...
my_model_freeq = wham::fit_wham(MakeADFun.silent = TRUE, input = input, do.retro = FALSE, do.osa = FALSE)
...
```{r}
wham_model = my_model_freeq#my_model_try#my_model_q1.56
# compare SSB ----
data.frame(year=arep$SSB[,1],'ssb'=arep$SSB[,2],mod = 'amak_baseline') %>%
  bind_rows(data.frame(year = input_data$years,
...
# compare recruitment ----
data.frame(year=arep$R[,1],'R'=arep$R[,2]*1e6,mod = 'amak_baseline') %>% 
  bind_rows(data.frame(year = input_data$years,
...
```{r}
wham_model = my_model_try#my_model_noRE#my_model_q1.56

as.data.frame(arep$N) %>% 
  rename(year=V1) %>% 
  pivot_longer(cols=-year,names_prefix = 'V') %>% 
  mutate(age=as.integer(name)-1,value=value*1e6,mod = 'amak_baseline') %>% 
  select(-name) %>% 
  bind_rows(as.data.frame(wham_model$report()$NAA*1e3) %>% 
              mutate(year=wham_model$years, mod = 'wham') %>% 
...
do.call('rbind',wham_model$rep$selAA) %>% as.data.frame() %>% 
  ...
  filter(year==min(year)) %>% 
  ggplot()+geom_line(aes(age,value))+facet_grid(year~type)+
     theme(strip.text.y = element_blank(),
                strip.background.y = element_blank(),legend.position='none',
                panel.spacing.y = unit(.01, 'cm'))

as.data.frame(arep$sel_fsh_1[,3:13]) %>% mutate(year=arep$Yr,type='fsh') %>%
  bind_rows(as.data.frame(arep$sel_ind_1[,3:13]) %>% mutate(year=arep$Yr,type='idx') ) %>% 
  pivot_longer(cols=-c(year,type),names_to = 'age',names_prefix = 'V',names_transform = as.integer) %>% 
    ggplot()+
...
do.call('rbind',wham_model$rep$selAA) %>% as.data.frame() %>% 
  ...
  pivot_longer(cols=-c(year,type),names_to = 'age',names_prefix = 'V',names_transform = as.integer) %>% 
  mutate(mod='wham') %>% 
  bind_rows(as.data.frame(arep$sel_fsh_1[,3:13]) %>% mutate(year=arep$Yr,type='fsh') %>%
            bind_rows(as.data.frame(arep$sel_ind_1[,3:13]) %>% mutate(year=arep$Yr,type='idx') ) %>% 
            pivot_longer(cols=-c(year,type),names_to = 'age',names_prefix = 'V',names_transform = as.integer) %>%                                                       mutate(mod='amak')) %>% 
    ggplot()+
...
### Explore what would be the closest to AMAK
...
show_selex(model = "double-normal", initial_pars = c(5, -1.5, 0.1, 5.5, -4, -1), ages = input_data$ages)
...
inputry <- prepare_wham_input(input_data, 
                                model_name='lo', 
                                 NAA_re = list(N1_model = 1,
                                ...
                                selectivity=list(model=c("double-normal",'age-specific'), 
                                  #re=sel_re[[m]],
                                  initial_pars=list(c(5, -1.5, 0.1, 5.5, -4, -5),
                                                      scales::rescale(colMeans(arep$sel_ind_1[,-c(1,2)])))),
                                                   # arep_dlog$sel_ind_1[1,3:13])),
                                       
                                catchability = list(initial_q = 1.56))


inputry <- prepare_wham_input(model_name = 'Initial',
                                basic_info = input_data,
                                # N1_model=1: 2 fixed effects parameters: an
                                # initial recruitment and an instantaneous
                                # fishing mortality rate to generate an
...
inputry <- prepare_wham_input(input_data,
                                ...
                                selectivity=list(model=c("age-specific","age-specific",'age-specific'), 
                                  initial_pars=list(scales::rescale(colMeans(arep$sel_ind_1[c(1:23),-c(1,2)])),
                                                    scales::rescale(colMeans(arep$sel_ind_1[c(23:46),-c(1,2)])),
                                                    arep$sel_ind_1[1,3:13]),
                                  fix_pars = list(2:11,2:11,1:11),
                                                   n_selblocks = 3),
                                       
                                catchability = list(initial_q = 1.56))
    
 # Fix some parameters:
inputry$map$logit_q = factor(NA)
inputry$map$log_N1_pars = factor(c(1,NA))

  # fit model
  try <- fit_wham(inputry, MakeADFun.silent = TRUE,do.check=T, do.osa=F, do.proj=F, do.retro=F)
...
for(m in 1:n.mods){
  if(sel_model[m] == "logistic"){ # logistic selectivity
    ...
    input_mods_dlog <- prepare_wham_input(input_data, 
